...
Для решения задачи воспользуемся языком программирования Python. Выбор обусловлен тем, что он поставляется в большинстве дистрибутивов Linux как один из инструментов по умолчанию. Это позволит запускать обработку лога прямо на web-сервере. Так же Python имеет мощные встроенные средства для работы с текстом и обширный набор подключаемых модулей.

В качестве хранилища данных для формирования витрины данных можно использовать что угодно, но аналитики любят SQL. Поэтому используем PostgreSQL.

Применение более развитых технологий обработки данных (Spark, Hadoop и пр.) целесообразно только при необходимости обработки логов размером в 100 Гб и более. Применение компилируемых языков программирования (C, C#, Java, Scala и пр.) может позволить ускорить обработку данных, но тогда может возникнуть проблема оперативной модификации алгоритма обратки и установки дополнительных библиотек, языковых средств.
...
* `\"%{User-agent}i\"` - идентификатор браузера
...
Разбору подлежат всего два параметра:
...
Данный разбор производится после применения регулярного выражения и выполняется в файле [apache_log_parser.py](src/log_parser/log_entity.py)

Уникальный идентификатор пользователя представляет собой IP адрес и имя пользователя, если он был авторизован web-сервером.

Информацию о агенте получим с помощью библиотеки разбора [user_agents](https://pypi.org/project/user-agents/)
...
6. Сбор информации о кодах ответов сервера по устройствам

### Обобщение результатов и запись в файл

Данная операция необходима, т.к. для витрины данных не нужны списки пользователей или какая-либо служебная информация. Поэтому перед записью данных в файл формируется агрегированная структура данных в формате JSON:
...
```
{
    ...
    "devices": {
        "Linux": { // - название устройства или ОС
            ...
            "not_200_answers": int - количество статусов ответов отличных от 200,
            "browsers": [ // - браузеры
                [
                    "AhrefsBot 6.1", // название браузера
                    354 // количество уникальны
                ],
            ],
            "request_answers": {
                "404": int - количество статусов ответа,
            },
        },
    }
}
...
Для загрузки обобщенного файла результатов разбора логов используется специальный загрузчик, который подключается к СУБД, формирует схему данных и загружает данные.
...
В результате выполнения запроса получим таблицу вида:
...
Для файла лога [https://disk.yandex.ru/d/BsdiH3DMTHpPrw](https://disk.yandex.ru/d/BsdiH3DMTHpPrw):

* всего полных записей: 10 365 077
* не полных/плохих записей, которые не получилось разобрать: 75 (<0.001 %)
* записей, относящихся к роботам: 1 109 418 (~10 %)
* уникальных пользователей: 258 595
* всего устрйоств: 2981
* "плохих" исправленных названий устройств: 1291 (<0.001 % от обработанных строк)
* "плохих" названий браузеров: 0

Итого. На файл мнее 0.001% "плохих" записей, которые не удалось обработать. 10% данных файла - поисковые роботы. Качество исходных данных - великолепное.

## Быстродействие

Быстродействие оценивалось временим обработки файла access.log объемом 3.26 ГБ на разном количестве (1-20) параллельных обработчиков.
...
Для автоматизированного получения результатов тестирования использовался скрипт [PowerShell](src/performance_test.ps1).

Результаты работы скрипта представлены в таблице. Из нее видно, что на физических ядрах производительность растет линейно.

| Количество обработчиков (потоков)  | Время, мин   |  Ускорение | Средняя скорость чтения файла, МБ/с |
|----|-------|------|-------|
| 1  | 12,98 | 1,00 | 4,29  |
| 2  | 6,77  | 1,92 | 8,22  |
| 3  | 4,57  | 2,84 | 12,17 |
...
Например, в текущем задании ничего не говорится о важности временных отметок, а их разбор занимает существенное время. Поэтому возможно в файле ["apache_log_parser.py"](src/log_parser/apache_log_parser.py) убрать разбор даты заменив строки:
```
...
| Количество обработчиков (потоков)  | Время с датами (А), мин | Время без дат (Б), мин  | Ускорение (А/Б) | Средняя скорость чтения файла, МБ/с |
|----|-------|------|------|--------|
